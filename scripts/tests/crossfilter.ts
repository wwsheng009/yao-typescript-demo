...
function main() {
  ...
  var firstSize = 9e4,
    ...
    randomHourOfDay = randomIndex([
      0, 0, 0, 0, 0, 0, 0, 0.2, 0.5, 0.7, 0.85, 0.9, 0.8, 0.69, 0.72, 0.8, 0.78,
      0.7, 0.3, 0, 0, 0, 0, 0
    ]),
  ...
  console.log(
    'Filtering by amount: ' + formatNumber((Date.now() - then) / k) + 'ms/op.'
  );
  amount.filterAll();
  ...
  days.all().forEach(function (g) {
    console.log(
      '       ' + formatDay(g.key) + ': ' + new Array(x(g.value) + 1).join('▇')
    );
  });
  console.log('');
...
function randomRecentDate(randomDayOfWeek, randomHourOfDay, weeks) {
  var now = Date.now();
  return function () {
    var d = d3.timeWeek.offset(new Date(), -Math.floor(Math.random() * weeks));
